...
ERROR_MESSAGE = "Oops! An exception occurred! \n\n**Error** : {} " \
            "\n\nTolong Laporan ke [Support](t.me/dost_hai_sab) if error " \
...
async def main(_, msg):
    await msg.reply(
        "Please press which string do you want to take",
...
async def generate_session(bot, msg, telethon=False):
    ...
    try:
        ...
        await api_id_msg.reply('API_ID (which must be an integer). Please start generating session again.', quote=True, reply_markup=InlineKeyboardMarkup(Data.generate_button))
        return
    api_hash_msg = await bot.ask(user_id, 'Please send `API_HASH`', filters=filters.text)
    if await cancelled(api_id_msg):
        return
    api_hash = api_hash_msg.text
    phone_number_msg = await bot.ask(user_id, 'NOW `PHONE_NUMBER` Dengan By Number Code. \nExample : `+91xxxxxxx`', filters=filters.text)
    if await cancelled(api_id_msg):
        return
    ...
    text = "**{} STRING SESSION** \n\n`{}` \n\nGenerated by @DEXDECRYPT_MUSIC".format("TELETHON" if telethon else "PYROGRAM", string_session)
    await client.send_message("me", text)
    await client.disconnect()
    await phone_code_msg.reply("Successfully Picked Up {} string session. \n\nPlease check on Saved Messages! \n\nBy @DEXDECRYPT_MUSIC".format("telethon" if telethon else "pyrogram"))
